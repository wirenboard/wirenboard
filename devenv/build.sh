...
do_build() {
	...
	time DEBIAN_RELEASE=$RELEASE DEBIAN_MIRROR=http://debian-mirror.wirenboard.com ARCH=$ARCH WB_RELEASE=$WB_RELEASE WB_COPY_QEMU=true /root/rootfs/create_rootfs.sh $BOARD $ADDITIONAL_REPOS
...
do_build_sbuild_env() {
	...
	#add conactless repo
	echo "deb http://deb.wirenboard.com/dev-tools experimental.trixie main" > ${ROOTFS}/etc/apt/sources.list.d/wirenboard-dev-tools.list
...
do_build trixie armhf 6x wb6 testing http://deb.wirenboard.com/all@experimental.trixie:main
do_build trixie arm64 8x wb8 testing http://deb.wirenboard.com/all@experimental.trixie:main
