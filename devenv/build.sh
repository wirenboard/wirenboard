...
do_build() {
	...
	time DEBIAN_RELEASE=$RELEASE ARCH=$ARCH WB_RELEASE=$WB_RELEASE DEFAULT_ADD_REPO_RELEASE=all DEFAULT_ADD_REPO_COMPONENT=experimental.trixie /root/rootfs/create_rootfs.sh $BOARD $ADDITIONAL_REPOS
