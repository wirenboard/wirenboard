...
WBDEV_TARGET_RELEASE=${WBDEV_TARGET_RELEASE:-""}
...
if [[ -z "$WBDEV_TARGET" ]] && [[ -z "$WBDEV_TARGET_RELEASE" ]]; then
    WBDEV_TARGET="current-$WBDEV_TARGET_ARCH"
    echo "Using WBDEV_TARGET=$WBDEV_TARGET by default. Set WBDEV_TARGET or WBDEV_TARGET_RELEASE to override."
fi
